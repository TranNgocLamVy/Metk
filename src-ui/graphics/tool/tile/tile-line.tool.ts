...
export class TileLineTool extends PointerTool {
    ...
    private previewPayloads: Map<string, TileLinePayload> = new Map();
    ...
    private createPayloads(origin: Coordinate): TileLinePayload[] {
        if (!this.currentView || !this.isTargetLayerSupported(this.targetLayerRenderer)) return [];

        const selectedTiles = getSelectedTiles(this.editorFacade);
        if (!selectedTiles) return [];

        const payloads: TileLinePayload[] = [];
    ...
    private createPayload(coordinate: Coordinate, tile: Tile): TileLinePayload | null {
        ...
        sprite.alpha = TileLineTool.spriteAlpha;
