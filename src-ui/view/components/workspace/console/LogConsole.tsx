    ...
    return (
        <ScrollArea className="h-full w-full">
            <VStack className="w-full gap-0">
                {logs.map((log) => (
                    <VStack key={log.id} className="w-full border-b border-border/50 p-2 text-sm hover:bg-foreground/5 transition-colors">
                        <HStack align="start" className="gap-2">
                            ...
                            <VStack className="gap-1 flex-1 min-w-0">
                                <HStack align="center" className="gap-2">
                                    ...
                                    <span className="text-foreground font-medium break-words">{log.message}</span>
                                </HStack>
                                ...
                                {log.actions && log.actions.length > 0 && (
                                    <HStack className="pt-1 gap-2 flex-wrap">
                                        {log.actions.map((act, idx) => (
                                            <Button key={idx} variant="outline" size="sm" onClick={act.onClick} className="h-6 text-xs px-2">
                                                {act.label}
                                            </Button>
                                        ))}
                                    </HStack>
