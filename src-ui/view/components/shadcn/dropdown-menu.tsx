...
function DropdownMenuContent({ className: customClassName, align = "start", sideOffset = 4, ...props }: React.ComponentProps<typeof DropdownMenuPrimitive.Content>) {
	...
	const className = "bg-surface-overlay text-foreground min-w-32 rounded-none shadow-lg duration-100 z-50 max-h-(--radix-dropdown-menu-content-available-height) w-(--radix-dropdown-menu-trigger-width) origin-(--radix-dropdown-menu-content-transform-origin) overflow-x-hidden overflow-y-auto";
	return (
...
function DropdownMenuItem({
	...
	const className = "gap-2 rounded-none px-2 py-2 text-foreground text-xs relative flex cursor-default items-center outline-hidden select-none focus:bg-accent focus:text-accent-foreground";
	return (
