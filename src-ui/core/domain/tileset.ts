...
export class Tileset extends BaseObject {
    ...
    constructor(tilesetData: TilesetData, texture: Texture | null) {
        ...
        if (tilesetData.tile.length > 0) {
            this.tiles = tilesetData.tile.map(tileData => {
                ...
                return new Tile(tileData, tileTexture);
            });
        } else {
            this.tiles = tileTextureList.map((texture, index) => {
                return new Tile({ id: index }, texture);
            });
        }
    }
    ...
    public static async loadTileset(filePath: string): Promise<Tileset | null> {
        ...
        if (!parseJSON.tileset.image) {
            return new Tileset(parseJSON.tileset, null);
        }
        ...
        return new Tileset(parseJSON.tileset, texture);
    }
}
